...
$business_tour = new BusinessTour($db);
    ...
    if (!empty($data->token)) {
        ...
        if (strval($number_token) == strval($data->cont_number)) {
            ...
            $booking_data = array(
                ...
                'special_note' => mysqli_real_escape_string($database->connect(), $data->spe_note),
            ...
            $result = $business_tour->create($booking_data);
            if ($result['status']) {
                http_response_code(200);
                echo json_encode(array("message" => "Your booking has been placed", "last_id" => $result['return_data'], "status" => true));
            } else {
