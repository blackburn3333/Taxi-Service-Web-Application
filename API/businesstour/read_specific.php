...
if (!empty($data->token)) {
    $token_data = $general->read_jwt($data->token);
    $number_token = str_replace('"', "", json_encode($token_data->data->id));
    $statement = $businessTour->read_spec($number_token);
    ...
    echo json_encode(array("message" => "Unable to find business in system"));
}
